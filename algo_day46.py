...
for t in range(int(input())):                              # T 입력받기
    N = int(input())                                       # N 입력받기     
    input_data = [input().split() for _ in range(N)]       # N번 알파벳과 숫자 쌍 입력 받기
    string = ''.join([a[0]*int(a[1]) for a in input_data]) # 알파벳*숫자 형태로 변환 후 하나의 문장으로 만들기
    print(f"#{t+1}")                                       # t 출력
    for i in range(0, len(string), 10):                    # index 10 단위로 돌리기
        print(string[i:i+10])                              # 문자열 10개씩 끊어서 출력
